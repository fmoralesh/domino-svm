...
void getProblemSVM(struct svm_problem* prob, int* training_labels, int n_dominos, float dominosID[][128]){
	int row_x = 0;
	int row_space = 0;
	int n_data = 0;
	int n=0;
	...
	// Assigning data values to svm_problem structure
	for(int j=0; j<n_dominos; j++){
		for(int m=0; m<128; m++){
			if(dominosID[j][m]!=0){
			row_space++;
			}
		...
		for(int m=0; m<128; m++){
			if(dominosID[j][m]!=0){
				...
				x_space[row_space].value = (double)dominosID[j][m];
				row_space++;
			}
		}
	...
	for(int j=0; j<n_dominos; j++){
		prob->y[row_x] = training_labels[j];
		row_x++;
	}
...
void bestParametersSVM(struct svm_problem prob, struct svm_parameter param, struct svm_grid* grid){
	...
	std::cout << "Best C: " << grid->best_c << " Best gamma: " << grid->best_g << std::endl;
